...
fn cli_converts_fixtures_and_derives_claims() {
    ...
    assert_eq!(ic.len(), 5);
    assert_eq!(output["public_signals_hex"].as_array().unwrap().len(), 4);
...
#[test]
fn cli_rarimo_mode_keeps_full_public_vector_and_derives_claims() {
    let output_path = unique_output_path();
    ...
    let status = Command::new(env!("CARGO_BIN_EXE_proof-adapter"))
        ...
        .arg(fixture("proof.json"))
        .arg("--vk")
        .arg(fixture("rarimo_query_vk.json"))
        .arg("--public")
        .arg(fixture("rarimo_query_public.json"))
        .arg("--out")
        .arg(&output_path)
        .arg("--rarimo-mode")
        .arg("--current-date")
    ...
    assert_eq!(
        output["verification_key"]["ic"].as_array().unwrap().len(),
        24
    );
    assert_eq!(output["claims"]["country_code"], 840);
    assert!(output["claims"]["is_human"].as_bool().unwrap());
